...
public class KeyFMapTest extends TestCase {
  ...
  private static void doTestGetKeys(int size) {
    ...
    for (int i = 0; i < size; i++) {
      int index = ArrayUtil.indexOf(keys, KEYS[i].hashCode());
      assertTrue("Key not found: " + KEYS[i], index >= 0 && index < size);
    }
  }

  private static void doTestGetValues(int size) {
    KeyFMap map = createKeyFMap(KEYS, VALUES, size);
    ...
    for (int i = 0; i < size; i++) {
      int index = ArrayUtil.indexOf(values, VALUES[i], 0, size);
      assertTrue("Value not found: " + VALUES[i], index >= 0);
    }
  }
  ...
  public void testGetValuesOnEmptyFMap() {
    doTestGetValues(0);
  }

  public void testGetValuesOnOneElementFMap() {
    doTestGetValues(1);
  }

  public void testGetValuesOnTwoElementFMap() {
    doTestGetValues(2);
  }

  public void testGetValuesOnArrayBackedFMap() {
    doTestGetValues(5);
  }

  public void testGetValuesOnMapBackedFMap() {
    doTestGetValues(10);
  }
}
